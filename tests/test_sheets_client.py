...
def test_get_or_create_summary_sheet_reinitializes_when_headers_missing():
    import gspread
    import sheets_client

    mock_spreadsheet = MagicMock()
    mock_ws = MagicMock()
    mock_ws.get_all_values.return_value = [["2024-05", "100.0", "90.0", "10.0"]]  # Missing header & total
    mock_spreadsheet.worksheet.return_value = mock_ws
...
@patch("sheets_client.config")
def test_sync_summary_sheet_scans_and_rewrites_all_month_tabs(mock_config, mock_get_gc):
    import sheets_client
    ...
    summary_ws.append_rows.assert_called_once_with(
        [
            ["Month", "Total Amount", "Total Net", "Stripe Fee"],
            ["Total", "=SUM(B3:B)", "=SUM(C3:C)", "=SUM(D3:D)"],
            ["2024-05", "=SUM('2024-05'!F2:F)", "=SUM('2024-05'!H2:H)", "=SUM('2024-05'!G2:G)"],
